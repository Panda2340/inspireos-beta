...
display = PicoGraphics(display=DISPLAY_PICO_DISPLAY, pen_type=PEN_P4, rotate=0)   #import the required libraries for the hardare

display.set_backlight(0.5)
display.set_font("bitmap8")

button_a = Button(12)
button_b = Button(13)             #set up buttons
button_x = Button(14)
button_y = Button(15)

WHITE = display.create_pen(255, 255, 255)  #set up colours
...
clear()
display.set_pen(WHITE)                                  #lines 35-48? boot sequence
...
if oobestatus == 1:
    ...
    bootnumcheck = 0
sysreg = ["buildnum:", buildnum, "bootcount:", bootcount, "oobestatus:", oobestatus, "bootnumcheck:", bootnumcheck, "-"]   #registry values
clear()
...
time.sleep(0.1)
while True:
    if button_a.read():                                  #57-69: menu script, open menu and draw contents.
        mode = 1
        msts = 1
        clear()                                           
        display.set_pen(WHITE)                           
        ...
        display.update()
    time.sleep(0.1)
    if button_y.read() and appsel == 1 and msts == 1:                          #Y button changes selected option in menu?
        if mode == 1:
            clear()                                           
            display.set_pen(WHITE)                           
            display.text("Menu", 10, 10, 240, 4)
        ...
        while True:
            ...
            shape = 1
            mode = 1
            if button_x.read() and appsel == 2:       #98-134: open demo app and play the demo
                clear()
    ...
    if msts == 2 and button_y.read():
        ...
        while True:
            ...
            shape = 1
            mode = 1
            if msts == 2 and appsel == 2 and button_x.read():  #script to display information of the hardware that InspireOS is being run on. (preset)
                clear()
